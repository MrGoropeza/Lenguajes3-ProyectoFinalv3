...
{
    public partial class RegisterPacientePage : System.Web.UI.Page
    {
        ...
        protected async void btn_register_Click(object sender, EventArgs e)
        {
            ...
            if (!encontrado_correo && !encontrado_dni && fecha_valida)
            {
                ...
                try
                {
                    string password = usuario.nombre[0].ToString() + usuario.apellido[0].ToString() + usuario.dni.ToString();

                    Consultorio.token = await Consultorio.auth.registrarUsuario(usuario.correo, password);
                    Consultorio.database.addUser(usuario);
                    GMailProvider.mandarInfoCuentaCreada(usuario,password);
                    Response.Redirect("ReservarAdminPage.aspx");
